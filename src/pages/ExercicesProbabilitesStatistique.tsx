...
// ce qui n'est pas ouvert est compté à partir des fiches signalétiques —
// jamais à partir du contenu, qui n'est pas dans les fichiers servis.
//
// ─── ÉTAT AU 2026-08-29 ────────────────────────────────────────────────────
//
// Zéro exercice ouvert : la sélection éditoriale n'a pas eu lieu et tout est
// réservé en attendant. Un visiteur sans accès voit donc les 451 fiches
// verrouillées et rien d'autre ; un détenteur d'accès reçoit la banque
// complète par la Cloud Function, comme pour le calcul différentiel.
...
import { CHAPITRES, TOTAL_BANQUE } from "../data/probabilites-statistique";
import {
  LIB_DIFFICULTE,
  LIB_TYPE,
  type Difficulte,
...
export default function ExercicesProbabilitesStatistique() {
  ...
  const banque = useExercicesComplets("probabilites-statistique");
  ...
  const acces = useAcces("probabilites-statistique");
    ...
    <div className="container-page py-12 sm:py-16">
      <AnimatedSection className="mx-auto max-w-3xl text-center">
        ...
        <p className="mt-4 text-balance text-lg text-ink-600">
          {accesResolu ? (
            <>
              {provenance === "bundle" && total === 0 ? (
                <>
                  <span className="font-semibold text-brand-900">{TOTAL_BANQUE} exercices</span>{" "}
                  corrigés, réservés aux détenteurs du matériel de cours. Chacun avec sa
                  réponse finale et sa démarche détaillée.
                </>
              ) : (
                <>
            ...
            {filtreProg === "completes"
              ...
              : filtreProg === "non-completes"
                ...
                : filtreProg === "marques"
                  ...
                  : provenance === "bundle" && total === 0
                    ? // Rien d'ouvert en vitrine : ce n'est pas le filtre qui
                      // est en cause, et inviter à l'élargir enverrait
                      // l'étudiant chercher ce qui n'existe pas.
                      "Les exercices de ce cours sont réservés aux détenteurs du matériel. Connecte-toi si tu y as accès."
