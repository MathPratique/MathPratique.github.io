...
// il suffira de changer l'id ci-dessous : les accents suivent.
const COULEUR = topics.find((t) => t.id === "differential-calculus")!.couleur;
...
export default function Boutique() {
  ...
  return (
    <div className="container-page py-12 sm:py-16">
      ...
      {/* ---------- 4.1  Accroche ---------- */}
      <AnimatedSection className="mx-auto mt-6 max-w-3xl text-center">
        <span className={`inline-block rounded-full px-4 py-1.5 text-sm font-semibold ${COULEUR.accentBadge}`}>
          Boutique
        </span>
      ...
      <AnimatedSection delay={0.1} className="mt-10">
        <div className="mx-auto max-w-2xl rounded-3xl border border-brand-100 bg-brand-50/40 p-8 text-center sm:p-10">
          ...
          <p className={`mt-2 text-sm font-medium ${COULEUR.accentTexte}`}>
            {PERIODE_LANCEMENT}
          </p>
          ...
          {PAIEMENT_ACTIF ? (
            ...
            <div className="mt-6">
              <button
                type="button"
                disabled
                className={`inline-flex w-full cursor-not-allowed items-center justify-center gap-2 rounded-full px-7 py-3 text-sm font-semibold sm:w-auto opacity-50 ${COULEUR.accentBouton}`}
              >
      ...
      <AnimatedSection delay={0.15} className="mt-16">
        ...
        <div className="mt-8 grid gap-5 sm:grid-cols-2">
          {CONTENU_PACKAGE.map((item) => (
            ...
            >
              <div className={`flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-xl ${COULEUR.accentBadge}`}>
...
function BoutonAchat() {
  ...
  if (etat.actif) {
    return (
      <div className="mt-6">
        <Link
          to="/mon-compte"
          className={`inline-flex w-full items-center justify-center gap-2 rounded-full px-7 py-3 text-sm font-semibold shadow-sm transition-colors duration-200 sm:w-auto ${COULEUR.accentBouton}`}
        >
  ...
  async function acheter() {
    ...
    if (!utilisateur) {
      navigate("/connexion", { state: { retour: "/boutique/calcul-differentiel" } });
      return;
    }
    setEnvoi(true);
    try {
  ...
  return (
    <div className="mt-6">
      <button
        ...
        onClick={acheter}
        disabled={envoi}
        className={`inline-flex w-full items-center justify-center gap-2 rounded-full px-7 py-3 text-sm font-semibold shadow-sm transition-colors duration-200 disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto ${COULEUR.accentBouton}`}
      >
